...
const globs = allartifacts.filter((dir) => fs.lstatSync(dir).isDirectory()).map((dir) => `${dir}/**/*.js.map`);

const files = globs.map((pattern) => glob.sync(pattern)).reduce((acc, arr) => acc.concat(arr), []);
